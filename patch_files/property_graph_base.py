# property_graph_base successfully patched.
# 2
import asyncio
...
class PropertyGraphIndex(BaseIndex[IndexLPG]):
    ...
    def _insert_nodes(self, nodes: Sequence[BaseNode]) -> Sequence[BaseNode]:
        ...
        for node in nodes:
            ...
            # Original dictionary (kg_rel properties)
            kg_rel_properties = kg_rels_to_insert[0].properties
        ...
        if self._embed_kg_nodes:
            ...
            #kg_relation_texts = [str(kg_rel) for kg_rel in kg_rels_to_insert]
            #change the kg_relation_text so they are made of the source_id, the relation name the target_id and the properties
            # excluding the properties in the exclude_keys list
            kg_relation_texts = []
            for kg_rel in kg_rels_to_insert:
                # get the source_id and target_id
                source_id = kg_rel.source_id
                target_id = kg_rel.target_id
                # get the relation name
                relation_name = kg_rel.label
                # get the properties
                kg_rel_properties = kg_rel.properties
                # Create a filtered dictionary excluding specified keys
                filtered_properties = {key: value for key, value in kg_rel_properties.items() if key not in exclude_keys}
                # create the new text
                kg_rel_text = f"{source_id} {relation_name} {target_id} {filtered_properties}"
            ...
            if self._use_async:
                kg_relation_embeddings = asyncio.run(
                    self._embed_model.aget_text_embedding_batch(
                        kg_relation_texts, show_progress=self._show_progress
                    )
                )
            else:
                kg_relation_embeddings = self._embed_model.get_text_embedding_batch(
                    kg_relation_texts,
                    show_progress=self._show_progress,
                )
