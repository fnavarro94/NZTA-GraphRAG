...
# max_depth: The maximum depth to search for paths
        ...
        for item in response:
            ...
            rel_id = item['rel_id']
            rel_type = item['type']
            # Check if seed node is part of the triplet
            if seed_id == source_id or seed_id == target_id:
                print("There will be single triplet results")
                # The path is just the triplet itself
                path_text = f"{source_id} -> {rel_type} -> {target_id}"
                results.append(path_text)
            else:
                print("There will be results from the second query")
                # Run the Cypher query to find the path
                response2 = self.structured_query(f"""
                // Match the seed node
                ...
                WHERE id(rel) = $rel_id AND type(rel) = $rel_type

                // Find all shortest paths from seed to target
                MATCH path = allShortestPaths((seed)-[*..10]-(target))
                WHERE ALL(r IN relationships(path) WHERE type(r) <> 'MENTIONS')
                AND ALL(r IN relationships(path) WHERE r.embedding IS NOT NULL AND size(r.embedding) = $dimension)

                // Calculate the path score based on embeddings
                WITH path, rel, target,
                    reduce(score = 0.0, r IN relationships(path) | score + vector.similarity.cosine(r.embedding, $embedding)) AS path_score

                // Check if the triplet relationship is already in the path
                WITH path, rel, path_score, target,
                    relationships(path) AS pathRels,
                    nodes(path) AS pathNodes,
                    ANY(r IN relationships(path) WHERE id(r) = $rel_id) AS tripletInPath

                // If the triplet is not in the path, append it
                WITH pathNodes, pathRels, rel, path_score, tripletInPath, target
                WITH CASE
                    WHEN tripletInPath THEN pathNodes
                    ELSE pathNodes + [target]
                    END AS fullNodes,
                    CASE
                    WHEN tripletInPath THEN pathRels
                    ELSE pathRels + [rel]
                    END AS fullRels,
                    path_score + CASE WHEN tripletInPath THEN 0.0 ELSE vector.similarity.cosine(rel.embedding, $embedding) END AS total_score

                // Order by total_score descending
                ORDER BY total_score DESC

                // Build the path text with correct arrow directions
                WITH fullNodes, fullRels, total_score

                WITH [i IN range(0, size(fullRels)-1) |
                    CASE
                        WHEN id(fullNodes[i]) = id(startNode(fullRels[i])) AND id(fullNodes[i+1]) = id(endNode(fullRels[i])) THEN
                            fullNodes[i].id + ' -> ' + type(fullRels[i]) + ' -> '
                        WHEN id(fullNodes[i]) = id(endNode(fullRels[i])) AND id(fullNodes[i+1]) = id(startNode(fullRels[i])) THEN
                            fullNodes[i].id + ' <- ' + type(fullRels[i]) + ' - '
                        ELSE
                            fullNodes[i].id + ' - ' + type(fullRels[i]) + ' - '
                    END
                ] + [fullNodes[-1].id] AS pathElements, total_score

                // Concatenate the path elements into a single string
                WITH reduce(s = '', element IN pathElements | s + element) AS path_text, total_score
        ...
        print("this is the length of the results")
        print(len(results))
        print("this is the length of the response")
        print(len(response))
        print("this are the full results")
        for res in results:
            print(res)
        ...
        for record in response:
            if record["type"] in ignore_rels:
                continue
            source_properties = remove_empty_values(record["source_properties"])
           
            source_properties['full_path'] = record['full_path']
            source = EntityNode(
                name=record["source_id"],
                label=record["source_type"],
            ...
            triples.append([source, rel, target])
            #print("this are source properties full path")
            #print(source.properties['full_path'])
